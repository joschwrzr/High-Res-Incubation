---
title: "Henrys Law on a High Resolution Incubation Set-up"
...
# High resolution incubation set up

To measure greenhouse gas emissions ($CO_2$ and $CH_4$) from soil samples, we are using an unconventional incubation set up, that allows us to follow gas production rates in a very high resolution. We use a [LICOR trace gas analyzer](https://www.licor.com/env/products/trace_gas/LI-7810), which is directly connected to the jar containing the soil sample.

![measurement set-up](input/Kruemel_3_Device.png){#id .class width=50% height=50%}

With this set up, we obtain high resolution measurements, which are much more reliable compared to manual samples, where only two samples are taken: one in the beginning of enclosure time and one at the end (which, for sure gives a beautiful linear regression). With our set up we obtain a continuous measurement series with a resolution of $\frac{1}{sec}$, for the whole enclosure time (figure below). However, one obstacle is that we actually don't see a linear increase, as you can see in the plot below: In the beginning of the measurement the slope is much higher than after ca. 9 minutes.
...
```{r measured, message=FALSE, warning=FALSE, fig.cap = "Single measurement of $CO_2$ concentration with fittet linear regression for ther first 9 minutes (white) and after the first 9 minutes (lila).", out.width = '50%'}
...
Let's look at the linear model in more detail, by producing a linear model for every 3 minutes of the measurement. Slope and intercept of all these models are shown below: The first model (no.1 in the y-axis) is the linear regression for the first 3 minutes, the 2nd for minute 3 to 6 and so on. On the x-axes intercept (left) and slope (right) of each model are plotted. The slope is decreasing in the first minutes, but after a certain time it stays stable (at around 0.021). This is the actual slope we are interested in. YAY!

But why is the slope different in the beginning? 
The blame is on Henry and his law.

```{r linearmodel, echo=FALSE,  message=FALSE, warning=FALSE,  fig.cap = "Linear model for every 3 minutes of $CO_2$ measured in a single measurement.", out.width = '50%'}
...
$$CO_{2 (g)} \rightarrow CO_{2 (aq)}$$


The ratio of the two rates **kh1** and **kh2** is described by Henry's Law : $K_H = \frac{kh1}{kh2}$
...
The rate constants **kh1** and **kh2** depend on how fast the $CO_2$ is transported to the gas-liquid phase boundary. When stirring, the rate constant **kh1** is faster, $CO_2$ outgases faster, and dilutes faster into the liquid phase (**kh2** is then faster, too).
...
$$\frac{dCO_{2(g)}}{dt} = kh1 \cdot CO_{2(aq)} \cdot \frac{V_w}{V_g} - kh2 \cdot CO_{2(g)} \cdot \frac{V_w}{V_g}$$
...
k1 =  (m * 1e-3 * 12/44 *1/P_TOC) * 1/day # abbaurate pro sec [1/s] 
tau = 1/k1 *1/day #abbaurate in tagen  [days]
...
G = (ppm_g *1e-6*101325)/(R*Temp*1000)  # CO2 in has [mol/l]

ppm_w = 450 # water phase eqilibrated to atmospheric CO2 in [ppm]
...
The yellow line is the ratio of the $CO_2$ amount in the gas and in the aqueous phase: after the first few minutes, it is nearly stable, because the equilibrium is reached.
