...
fontsize: 10pt
resource_files:
- EGU/vPICO/input/Kruemel_3_Device.png
---
...
We are using an unconventional incubation set up, that allows us to measure $CO_2$ production as it is produced. We use a [LICOR trace gas analyzer](https://www.licor.com/env/products/trace_gas/LI-7810), which is directly connected to the jar containing the soil sample.
...
```{r measured, message=FALSE, warning=FALSE, out.width = '50%'}
#setwd("~/Documents/Studium/Geographische Umweltforschung/AWI/Treat/Siikaneva/EGU/vPICO/")
...
Let's look at the linear models in more detail, by producing a linear model for every 3 minutes of the measurement. Slope and intercept of all these models are shown below: The first model (no.1 in the y-axis) is the linear regression for the first 3 minutes, the 2nd for minute 3 to 6 and so on. On the x-axes intercept (left) and slope (right) of each model are plotted. The slope is decreasing in the first minutes, but after a certain time it stays stable (at around 0.021). This is the actual slope we wanted to see. YAY!

But why is the slope different in the beginning? 
The blame is on Henry and his law.

```{r linear model, echo=FALSE,  message=FALSE, warning=FALSE, out.width = '50%'}
